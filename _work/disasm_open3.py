...
sys.stdout = io.open(r"E:\game\_work\open3_out.txt", "w", encoding="utf-8")
...
def disasm(start, count=130, label=""):
    print(f"\n========== {label} 0x{start:08X} ==========")
    o=va2off(start)
    if o is None: print("bad"); return
    n=0
    for ins in md.disasm(raw[o:o+count*7], start):
        print(f"  0x{ins.address:08X}: {ins.mnemonic:8} {ins.op_str}{ann(ins)}")
        n+=1
        if n>=count: break
        if ins.mnemonic=='ret': break
# archive open (Agent A: 0x4547E0). Find path source + failure returns.
disasm(0x4547E0, 110, "ARCHIVE_OPEN 0x4547E0")
# the CreateFileA wrapper used (0x4583CE region)
disasm(0x4583CE, 40, "openfile 0x4583CE")
# the function that builds archive name (called at 0x454800 -> 0x455849)
disasm(0x455849, 70, "buildname 0x455849")
print("done")
