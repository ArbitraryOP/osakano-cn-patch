...
sys.stdout = io.open(r"E:\game\_work\verify2_out.txt", "w", encoding="utf-8")
...
def disasm(start, count=120, label=""):
    print(f"\n========== {label} 0x{start:08X} ==========")
    o=va2off(start)
...
# Full install-check function (contains InstallDir/VoiceInstall reads). Find its start before 0x43FA50.
disasm(0x43F9C0, 200, "install-check (find voice path/flag use)")
print("done")
