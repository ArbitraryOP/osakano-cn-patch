  ...
  public static bool Bind(int pid){
    ...
    for(int d=0; d<nd; d++){ IMMDevice dev; coll.Item(d,out dev); object o;
      ...
      for(int i=0;i<ns;i++){ IAudioSessionControl sc; if(se.GetSession(i,out sc)!=0) continue;
        ...
        if(p==pid){ meter=(IAudioMeterInformation)sc; return true; } } }
    return false;
  }
...
function Click($h,$fx,$fy){ $c=New-Object W+RECT;[W]::GetClientRect($h,[ref]$c)|Out-Null
  ...
  [W]::SetForegroundWindow($h)|Out-Null;[W]::SetCursorPos($p.X,$p.Y)|Out-Null;Start-Sleep -Milliseconds 90
  [W]::mouse_event($MD,0,0,0,[IntPtr]::Zero);Start-Sleep -Milliseconds 60;[W]::mouse_event($MU,0,0,0,[IntPtr]::Zero) }
function Shot($h,$path){ $r=New-Object W+RECT;[W]::GetWindowRect($h,[ref]$r)|Out-Null
  $w=$r.R-$r.L;$ht=$r.B-$r.T; if($w -le 0){return}; $bmp=New-Object Drawing.Bitmap $w,$ht;$g=[Drawing.Graphics]::FromImage($bmp)
...
Write-Host "pid=$pid0 advancing prologue deep, continuous peak sampling"
$overall=0.0; $firstHitLine=-1
for($k=0;$k -lt 40;$k++){
  [W]::SetForegroundWindow($h)|Out-Null
  [W]::keybd_event(0x0D,0,0,[IntPtr]::Zero);Start-Sleep -Milliseconds 40;[W]::keybd_event(0x0D,0,2,[IntPtr]::Zero)
  Click $h 0.5 0.62
  # continuous sample ~1.3s
  $lineMax=0.0; $end=(Get-Date).AddMilliseconds(1300)
  while((Get-Date) -lt $end){ $p=[M]::Peak(); if($p -gt $lineMax){$lineMax=$p}; Start-Sleep -Milliseconds 25 }
  if($lineMax -gt $overall){$overall=$lineMax}
  if($lineMax -gt 0.01 -and $firstHitLine -lt 0){ $firstHitLine=$k }
  if($k % 8 -eq 0 -or $lineMax -gt 0.01){ Write-Host ("line {0,2}: peak={1:N4}" -f $k,$lineMax) }
  if($k -eq 12){ Shot $h "$OUT\deep_12.png" }
  if($k -eq 30){ Shot $h "$OUT\deep_30.png" }
}
Shot $h "$OUT\deep_end.png"
Write-Host ("OVERALL peak over 40 lines = {0:N4}  firstAudibleLine={1}  ({2})" -f $overall,$firstHitLine, $(if($overall -gt 0.01){'AUDIBLE'}else{'SILENT - no BGM/voice in prologue'}))
Write-Host 'DEEP ADVANCE DONE'
